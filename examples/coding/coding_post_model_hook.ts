...
import { CodingAgentStateHelpers, type CodingAgentStateType } from "./coding_agent_state.js";
...
export function createCodingAgentPostModelHook() {
   ...
   */
  async function postModelHook(state: StateType, model?: any): Promise<StateType> {
    ...
    if (!(lastMessage instanceof AIMessage || lastMessage instanceof AIMessageChunk) || !lastMessage.tool_calls) {
      return state;
    }
    ...
    for (const toolCall of lastMessage.tool_calls) {
      ...
      if (WRITE_COMMANDS.has(toolName)) {
        ...
        if (CodingAgentStateHelpers.isOperationApproved(state, toolName, toolArgs)) {
          approvedToolCalls.push(toolCall);
        } else {
          const approvalKey = CodingAgentStateHelpers.getApprovalKey(toolName, toolArgs);
          ...
          if (isApproved) {
            CodingAgentStateHelpers.addApprovedOperation(state, toolName, toolArgs);
            approvedToolCalls.push(toolCall);
          } else {
            continue;
          }
    ...
    if (approvedToolCalls.length !== lastMessage.tool_calls.length) {
      ...
      const MessageClass = lastMessage instanceof AIMessageChunk ? AIMessageChunk : AIMessage;
