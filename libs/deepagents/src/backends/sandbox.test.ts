...
class MockSandbox extends BaseSandbox {
  ...
  async execute(command: string): Promise<ExecuteResponse> {
    ...
    if (command.includes('-maxdepth 1') && command.includes('stat -c')) {
      ...
      const output = files.map((f) => `${this.files.get(f)!.length}\t${now}\tregular file\t${f}`).join('\n');
...
describe('BaseSandbox', () => {
  ...
  describe('read_file tool with sandbox backend', () => {
    it('should surface exactly one line-number column per line (backend read is raw; tool adds numbering once)', async () => {
      ...
      const readFileTool = (middleware as { tools: StructuredTool[] }).tools.find((t) => t.name === 'read_file');
