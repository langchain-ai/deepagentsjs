...
- **Fork point (merge-base):** `9221c8a2` (`chore(deepagents): move required ptc tools to metadata (#549)`)

All paths below are under `libs/deepagents/src/`. To regenerate the raw diff:

```bash
git fetch upstream
git diff 9221c8a2 HEAD -- libs/deepagents/src
```
...
### 6. Dependency: `zod` — declared pin is stale (actually runs zod 4.4.3)

**Files:** `libs/deepagents/package.json`, plus import changes in
`middleware/fs.ts`, `testing/utils.ts`, `agent.test-d.ts`

What the fork commits did:
- `libs/deepagents/package.json`: `zod` `^4.3.6` → `3.25.76` (exact pin).
- Some imports changed `"zod/v4"` → `"zod"` (the three files above). Note the
  two **most important** forked files — `middleware/subagents.ts` and
  `middleware/todos.ts` — still import `"zod/v4"`.
